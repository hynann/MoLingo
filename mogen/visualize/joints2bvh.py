...
import visualize.Animation as Animation

from visualize.InverseKinematics import BasicInverseKinematics, BasicJacobianIK, InverseKinematics
from visualize.Quaternions import Quaternions
import visualize.BVH_mod as BVH
from visualize.remove_fs import *

import torch

from visualize.utils.quat import ik_rot, between, fk, ik
...
class Joint2BVHConvertor:
    def __init__(self):
        self.template = BVH.load('./visualize/data/template.bvh', need_quater=True)
    ...
    def convert(self, positions, filename, iterations=10, foot_ik=True, save_file=True):
        '''
        ...
        if filename is not None and save_file:
            BVH.save(filename, new_anim, names=new_anim.names, frametime=1 / 20, order='zyx', quater=True)
        return new_anim, glb
